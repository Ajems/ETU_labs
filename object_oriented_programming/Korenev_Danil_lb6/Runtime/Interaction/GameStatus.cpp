...
bool GameStatus::isWin(Field* field, Player* player) const{
    bool win = ((player->getCoins() >= 10*field->getTotalCoins()) and (field->getPlayerPosition().first==field->getExitPosition().first and field->getPlayerPosition().second==field->getExitPosition().second));
    return win;
}
